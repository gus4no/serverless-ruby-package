...
class PackageRubyBundlePlugin {
  ...
  beforePackage() {
    ...
    const gemRoot = 'vendor/bundle/ruby/2.5.7'; //TODO: infer? configure?
    const extensionDir = `${gemRoot}/extensions/x86_64-linux/2.5.7-static`;
    ...
    const output = execSync('bundle exec ruby', {
      input: identifyGemsScript,
      env: bundleEnv,
    });
    const gems = JSON.parse(output);

    if (gems.some((x) => x.extensions)) {
      // puts Gem.extension_api_version # => 2.5.7-static
    ...
    gems.forEach((gem) => {
      ...
      if (gem.extensions) {
        this.serverless.service.package.include.push(
          `${extensionDir}/${gem.name}/**`
        );
      }
      ...
      this.serverless.service.package.include.push(
        `!${gemRoot}${gem.path}/.git/**`
      );
      if (excludeGemTests) {
        this.serverless.service.package.include.push(
          `!${gemRoot}${gem.path}/test/**`
        );
        this.serverless.service.package.include.push(
          `!${gemRoot}${gem.path}/spec/**`
        );
      }
    });
  }
  ...
  nativeLinuxBundle() {
    ...
    execSync(
      `docker run --rm -v "${localPath}:/var/task" lambci/lambda:build-ruby2.5 bundle install --standalone --path vendor/bundle`
    );
  }
  ...
  warnOnUnsupportedRuntime() {
    ...
    if (this.serverless.service.provider.name != 'aws') {
      this.log(
        `WARNING: serverless-ruby-package has only been tested with the AWS provider. It may not work with ${
          this.serverless.service.provider.name
        }, but bug reports are welcome.`
      );
      return;
    }
    if (this.serverless.service.provider.runtime != 'ruby2.5') {
      this.log(
        `WARNING: serverless-ruby-package has only been tested with the ruby2.5 runtime. It may not work with ${
